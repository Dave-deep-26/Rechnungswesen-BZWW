...
(function (global) {
  ...
  async function statusLaden() {
    var r = await client.rpc('zugang_status');
  ...
  async function anmelden(email, code) {
    ...
    var pruef = await client.rpc('code_pruefen', { p_code: code, p_email: email });
    ...
    if (status === 'eigener') {
      ...
      if (reg.error) {
        if (/already|registered|exists/i.test(reg.error.message)) {
          throw new Error('Diese E-Mail ist schon registriert. Melde dich mit deinem ersten Code an. Einen neuen Code kannst du danach im Konto einlösen.');
        }
      ...
      var einl = await client.rpc('code_einloesen', { p_code: code });
